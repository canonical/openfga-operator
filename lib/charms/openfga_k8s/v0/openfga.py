...
class OpenfgaProviderAppData(DatabagModel):
    ...
    address: str = Field(description="The openfga server address")
    scheme: Literal["http", "https"] = Field(description="The openfga server allowed schemes")
    port: str = Field(description="The openfga server port")
...
class OpenFGAProvider(Object):
    ...
    def update_server_info(self, address: str, scheme: str, port: str, dns_name: str):
        ...
        data = OpenfgaProviderAppData(address=address, scheme=scheme, port=port, dns_name=dns_name)
