...
@pytest_asyncio.fixture
async def http_ingress_url(http_ingress_integration_data: Optional[dict]) -> Optional[str]:
    ...
    assert matched is not None, "ingress url not found in http ingress integration data"

    return matched.group("ingress_url")


@pytest_asyncio.fixture
async def grpc_ingress_url(grpc_ingress_integration_data: Optional[dict]) -> Optional[str]:
    if not grpc_ingress_integration_data:
        return None

    ingress = grpc_ingress_integration_data["ingress"]
    matched = INGRESS_URL_REGEX.search(ingress)
    assert matched is not None, "ingress url not found in grpc ingress per unit integration data"

    return matched.group("ingress_url")


@pytest_asyncio.fixture
async def http_ingress_ip(http_ingress_url: Optional[str]) -> Optional[str]:
    if not http_ingress_url:
        return None

    ingress_ip, *_ = http_ingress_url.rsplit(sep=":", maxsplit=1)
    return ingress_ip


@pytest_asyncio.fixture
async def grpc_ingress_ip(grpc_ingress_url: Optional[str]) -> Optional[str]:
    if not grpc_ingress_url:
        return None

    ingress_ip, *_ = grpc_ingress_url.rsplit(sep=":", maxsplit=1)
    return ingress_ip
